...
def learn_emb(MethObj, di_graph, params, res_pre, m_summ):
    if params["experiments"] == ["lp"]:
        ...
        if not bool(int(params["load_emb"])):
            ...
            with open('%s_%s.emb' % (res_pre, m_summ), 'wb') as fp:
                pickle.dump(X, fp)
            with open('%s_%s.learnT' % (res_pre, m_summ), 'wb') as fp:
                pickle.dump(learn_t, fp)
        else:
            with open('%s_%s.emb' % (res_pre, m_summ),'rb') as fp:
                X = pickle.load(fp)
            try:
                with open('%s_%s.learnT' % (res_pre, m_summ), 'rb') as fp:
                    learn_t = pickle.load(fp)
...
def call_exps(params, data_set):
    ...
    di_graph = nx.read_gpickle('gem/experiments/gem/data/%s/graph.gpickle' % data_set)
...
def explict_run(data='benchmark_barabasi_albert_graph_m=8_n=1024',meth='aa',dim=128 ,rounds=1,s_sch=rw,exp=['gr','lp']):
    ''' Sample usage
