...
double get_avg_deg(deque<set<int> > & E, const deque<deque<int> > & member_list){
    int edges=0;
    ...
    double res = double(edges)/num_nodes;
    cout << "RES : " << res;
	return res;
}
...
//    char* avg_deg = (char*)get_avg_deg(E,member_list);
    double avg_deg = get_avg_deg(E,member_list);
    const char *fileName = "/home/ankita/studies/usc_research/GEM-benchmark/gem/experiments/config/synthetic/lfr_avgDeg.txt";
    bool file_exists = is_file_exist(fileName);
    if(file_exists == 0){
        ofstream myfile;
