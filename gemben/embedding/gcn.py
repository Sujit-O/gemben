...
class GCN(StaticGraphEmbedding):
    ...
    """`Graph Convolutional Network`_.

    Graph Convolutional Network (GCN) aggregate the neighbors embeddings 
    and features via convolutional operators, including spatial or 
    spectral filters. GCNs learn embeddings in a semi-supervised manner 
    and have shown great improvement and scalability on large graphs compared 
    to other methods.
    ...
    Examples:
        >>> from gemben.embedding.gcn import GCN
        ...
        >>>  embedding = GCN(d=128, beta=5, alpha=1e-5, nu1=1e-6, nu2=1e-6,
                               K=3, n_units=[500, 300, ],
                               n_iter=30, xeta=1e-3,
                               n_batch=500,
